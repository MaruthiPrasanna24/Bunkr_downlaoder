...
# Comment out moviepy and opencv imports to avoid memory issues
# They are disabled to prevent RAM exceed on large videos
...
def get_video_duration(video_path: str) -> int:
    """Returns video duration in seconds - only ffprobe, no heavy libs"""
    ...
    # No fallback to moviepy or opencv to avoid memory issues
    logger.warning(f"[v0] No duration available without ffprobe")
    return None
...
async def generate_video_thumbnail(video_path: str, output_path: str) -> bool:
    """Generate thumbnail using ffmpeg or fallback - no heavy libs"""
    if not os.path.exists(video_path):
...
async def download_and_send_file(client: Client, message: Message, url: str, session: requests.Session):
    try:
        ...
        for idx, item in enumerate(items, 1):
            ...
            # Skip processing if file is too large (>500MB) to avoid potential memory issues
            if file_size > 500 * 1024 * 1024:
                logger.warning(f"[v0] File too large ({human_bytes(file_size)}), skipping metadata/thumbnail to save memory")
                duration = None
                width = None
                height = None
                thumb_path = None
