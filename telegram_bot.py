...
async def download_and_send_file(client: Client, message: Message, url: str, session: requests.Session):
    ...
    try:
        ...
        if is_direct:
            ...
            for theItem in theItems:
                ...
                if box:
                    ...
                    if direct_item and isinstance(direct_item, dict) and isinstance(direct_item.get('url'), str):
                        items.append(direct_item)
                    else:
                        logger.warning(f"[PARSING] Invalid direct_item for {name}: {direct_item}")
        if not items:
        ...
        for idx, item in enumerate(items, 1):
            try:
                logger.info(f"[ITEM TYPE CHECK] Item type: {type(item)} - {item}")
                if not isinstance(item, dict):
                    logger.error(f"[ITEM ERROR] Item is not a dict: {item}")
                    continue
                file_url = item.get("url")
                file_name = item.get("name", album_name)
                if not isinstance(file_url, str):
                    logger.error(f"[ITEM ERROR] file_url is not a string: {file_url}")
                    continue
