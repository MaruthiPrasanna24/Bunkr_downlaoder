...
def get_real_download_url(session, url, is_bunkr=True, item_name=None):
    ...
    try:
        if is_bunkr:
            ...
            domain = parsed_url.hostname if parsed_url.hostname else 'bunkr.sk'
            url = f"https://{domain}{parsed_url.path}" if not parsed_url.scheme else url
            match = re.search(r'\/(i|v|d|f)\/(.*?)$', url)
            if not match:
                logger.error(f"[GET REAL URL] No slug found in URL: {url}")
                return None
            slug = unquote(match.group(2))
            api_url = f"https://{domain}/api/vs"
            encryption_data = get_encryption_data(session, slug, api_url)
            if not encryption_data:
                return None
            decrypted_url = decrypt_encrypted_url(encryption_data)
            logger.info(f"[GET REAL URL] Got decrypted URL for {item_name}: {decrypted_url}")
            return {'url': decrypted_url, 'size': -1, 'name': item_name}
        else:
            url = url.replace('/f/', '/api/f/')
            logger.info(f"[GET REAL URL] Fetching Cyberdrop: {url}")
            r = session.get(url, timeout=30)
            if r.status_code != 200:
...
def get_encryption_data(session, slug, api_url):
    """Get encryption data for Bunkr URLs"""
    try:
        logger.info(f"[ENCRYPTION] Getting data for slug: {slug} using API: {api_url}")
        r = session.post(api_url, json={'slug': slug}, timeout=10)
        if r.status_code != 200:
...
def decrypt_encrypted_url(encryption_data):
    ...
    try:
        ...
        logger.info(f"[DECRYPTION] Successfully decrypted URL: {decrypted_url}")
